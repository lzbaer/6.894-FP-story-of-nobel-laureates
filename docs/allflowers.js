...
var indivPetalColors = d3.scaleOrdinal()
  ...
  .range(['#FFB09E', '#CBF2BD', '#AFE9FF', '#FFC8F0', '#FFF2B4', '#82b4ff']);
...
d3.json('allflowerdata.json', function(laureates) {
  ...
  ******************************************************/
  ...
  var legendPetalColors = legend.append('g')
    .selectAll('g').data(["chemistry", "medicine", "physics", "literature", "peace", "economics"])
    .enter().append('g')
    .attr('transform', function(d, i) {    
      if (i < 3) {
        var x = i * (flowerSize / 3) + 80;
        var y = flowerSize/4 + 40;
      }
      else {
        var x = (i-3) * (flowerSize / 3) + 80;
        var y = flowerSize/2 + 50;
      }
      return 'translate(' + [x, y] + ')';
    });
  ...
  var legendBgColors = legend.append('g')
   ...
   //        'translate(' + [legendWidth / 2, flowerSize * .9] + ')')
    ...
    .attr('transform', function(d, i) {
      ...
      return 'translate(' + [x, flowerSize/4 + 180] + ')';
    });
  ...
  var legendLeaf = legend.append('g')
    ...
    .attr('transform', function(d, i) {
      ...
      return 'translate(' + [x, flowerSize/4 + 190] + ')'
    });
...
function filterFunction(d, i, include, age_vals, year_vals, fieldsInclude) {
  var included;
  if (d.age >= age_vals[0] 
    && d.age <= age_vals[1]
    && d.year >= year_vals[0]
...
function goButton() {
  ...
  flowers
    .filter((d, i) => filterFunction(d, i, true, age_vals, year_vals, fieldsInclude))
  ...
  flowers
    .filter((d, i) => filterFunction(d, i, false, age_vals, year_vals, fieldsInclude))
    .attr("opacity", 0)
